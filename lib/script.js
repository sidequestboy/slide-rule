...
function main() {
  ...
  innerG.append("g").classed("inner-axis", true).classed("labeled-axis", true).call(innerAxes[0]);
  innerG.append("g").classed("inner-axis", true).call(innerAxes[1]);
  innerG.append("g").classed("inner-axis", true).call(innerAxes[2]);
  const outerG = containerG.append("g").classed("outer", true);
  outerG.append("g").classed("outer-axis", true).classed("labeled-axis", true).call(outerAxes[0]);
  outerG.append("g").classed("outer-axis", true).call(outerAxes[1]);
  outerG.append("g").classed("outer-axis", true).call(outerAxes[2]);
  ...
  function setAxesAngle(angle, outer) {
    ...
    g.selectAll(`.${className}`).transition().duration(transitionTime).attr("transform", `rotate(${animationAngle})`).end().then(() => {
      d3.selectAll(`.${className}`).each((d, i, nodes) => {
        d3.select(nodes[i]).attr("transform", null).call(axes[i]);
      });
    });
  }
