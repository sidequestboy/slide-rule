...
let ticks = {
    ...
    2: _.range(scaleMin, scaleMid, majorTickStep / 25).concat(
...
function createInnerAxes() {
    return [
        createAxis(innerScale, ticks[1], 24, innerScaleFactor, radius, false),
        createAxis(innerScale, ticks[2], 6, innerScaleFactor, radius, false),
        createAxis(innerScale, ticks[3], 12, innerScaleFactor, radius, false)
    ]
}
...
function createOuterAxes() {
    return [
        createAxis(outerScale, ticks[1], 24, outerScaleFactor, radius, true),
        createAxis(outerScale, ticks[2], 6, outerScaleFactor, radius, true),
        createAxis(outerScale, ticks[3], 12, outerScaleFactor, radius, true)
    ]
}
